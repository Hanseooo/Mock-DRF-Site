...
window.onload = async () => {
  if (user) {
    welcomeHeader!.textContent = `Hello ${user} 🐾`; // Adding paw emoji
  } else {
    welcomeHeader!.textContent = `You are a Guest 🐾`; // Adding paw emoji
  }
};
  ...
  try {
    ...
    userList.forEach((username) => {
      ...
      li.className = "list-group-item bg-dark-subtle mt-1 border-light";
...
signInForm?.addEventListener("submit", async (e) => {
  ...
  const username =
    document.querySelector<HTMLInputElement>("#sign-in-username")!.value;
  const password =
    document.querySelector<HTMLInputElement>("#sign-in-password")!.value;
